...
public static class DistributedTracingExtensions
{
    ...
    {
        // Only map Prometheus scraping endpoint if OpenTelemetry MeterProvider is registered.
        // MeterProvider is only registered when DistributedTracing:Enable = true.
        var meterProvider = app.Services.GetService(typeof(OpenTelemetry.Metrics.MeterProvider));
