...
public static class SerilogLoggingExtensions
{
    ...
    {
        // Only use Serilog request logging if DiagnosticContext is registered.
        // DiagnosticContext is only registered when Logging:Serilog:Enable = true.
        var diagCtx = app.Services.GetService(typeof(Serilog.Extensions.Hosting.DiagnosticContext));
        if (diagCtx is null) return app;
